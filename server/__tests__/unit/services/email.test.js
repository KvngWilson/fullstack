...
describe('Email Service - Unit Tests', () => {
  ...
  describe('Email Templates', () => {
    ...
    describe('render method', () => {
      ...
      it('renders order status update templates used by order events', () => {
        const templateNames = [
          'orderReceived',
          'orderProcessing',
          'paymentConfirmed',
        ...
        templateNames.forEach((templateName) => {
          const template = templateService.render(templateName, {
            orderNumber: 'ORD-123',
            orderUrl: 'https://example.com/orders/ORD-123',
          });

          expect(template.subject).toContain('ORD-123');
          expect(template.html).toContain('https://example.com/orders/ORD-123');
          expect(template.text).toContain('ORD-123');
        });
      });
    });
  });
