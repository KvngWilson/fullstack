...
describe('Complete Backend E2E - Core Smoke Flow', () => {
  ...
  test('customer can register and login via platform auth', async () => {
  ...
  test('catalog endpoints are reachable', async () => {
  ...
  test('cart and ordering endpoints enforce auth and/or respond consistently', async () => {
    const cartNoAuth = await request(app).get('/api/v1/cart');
  ...
  test('admin endpoints are protected or unavailable', async () => {
    const adminRoutes = [
