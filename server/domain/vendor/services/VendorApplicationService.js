...
class VendorApplicationService {
  ...
  static async submitApplication(applicationData) {
    ...
    if (existingSlug.rowCount > 0) {
      throw new Error("Store name already taken. Please choose a different name.");
    }
  ...
  static async getApplications(filters = {}) {
    ...
    // Get total count
    let countQuery = "SELECT COUNT(*)::int as count FROM vendor_applications WHERE 1=1";
    const countParams = [];
  ...
  static async approveApplication(applicationId, reviewerId, notes = "") {
    ...
    try {
      ...
      const vendorResult = await client.query(
        ...
        [user.id, application.store_name, application.slug, application.business_name || ""]
      );
      ...
      try {
        await sendEmailJob({
          ...
          templateData: {
            ...
            dashboardUrl: `${process.env.APP_URL || "http://localhost:3000"}/vendor/dashboard`,
          },
        });
      } catch (emailError) {
    ...
    try {
      await sendEmailJob({
        ...
        templateData: {
          storeName: application.store_name,
          reason,
          reapplyUrl: `${process.env.APP_URL || "http://localhost:3000"}/vendor/apply`,
        },
      });
    } catch (emailError) {
    ...
    try {
      await sendEmailJob({
        ...
        templateData: {
          ...
          contactPerson: application.contact_person,
          message,
          updateUrl: `${process.env.APP_URL || "http://localhost:3000"}/vendor/application/${applicationId}`,
        },
      });
    } catch (emailError) {
