class PaymentValidationPolicy {
	...
	static validateRefundAmount(paymentAmount, alreadyRefunded, requestedAmount) {
		...
		if (!Number.isFinite(requested) || requested <= 0) {
			return { valid: false, message: "requested refund amount must be greater than 0" };
		}

		const remaining = Math.max(0, paid - refunded);
		if (requested > remaining) {
			return { valid: false, message: `requested refund exceeds remaining balance (${remaining})` };
		}
