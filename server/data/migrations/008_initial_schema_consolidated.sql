-- Migration: Consolidated Initial Schema (v2)
-- Purpose: Single baseline migration that reflects the effective schema after 001-007,
-- aligned with data/seed.sql and application usage.
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  payment_status payment_status_v2 NOT NULL DEFAULT 'pending',
  ...
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  deleted_at TIMESTAMPTZ,
  CONSTRAINT orders_valid_total CHECK (total >= 0),
  CONSTRAINT orders_valid_subtotal CHECK (subtotal_amount >= 0 OR subtotal_amount IS NULL),
  CONSTRAINT orders_valid_tax CHECK (tax_amount >= 0 OR tax_amount IS NULL),
  CONSTRAINT orders_valid_discount CHECK (discount_amount >= 0 OR discount_amount IS NULL),
  CONSTRAINT orders_valid_shipping CHECK (shipping_cost >= 0 OR shipping_cost IS NULL)
);
...
CREATE TABLE IF NOT EXISTS order_items (
  ...
  price_at_time NUMERIC(12,2) CHECK (price_at_time >= 0),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
