 ...
 * - ALERT_ENABLED: Enable/disable alerts (default: true in production)
 */
...
async function sendSlackAlert(alert) {
  if (!config.slackWebhookUrl) {
    structuredLogger.debug('Slack webhook URL not configured, skipping Slack alert');
...
async function sendPagerDutyAlert(alert) {
  if (!config.pagerDutyRoutingKey) {
    structuredLogger.debug('PagerDuty routing key not configured, skipping PagerDuty alert');
    return { success: false, reason: 'not_configured' };
  }
...
async function sendEmailAlert(alert) {
  if (!config.opsAlertEmail) {
    structuredLogger.debug('Ops alert email not configured, skipping email alert');
    return { success: false, reason: 'not_configured' };
  }
...
async function sendOpsAlert(alert) {
  ...
  if (!config.enabled) {
    structuredLogger.info('Alerts disabled, logged only', { title: alert.title });
  ...
  const allFailed = channels.length === 0 && failures.some(f => f.reason !== 'not_configured');
