import reducer, { setUser, clearUser, clearError } from '@/features/auth/authSlice';
import {
  loginThunk,
  registerThunk,
  logoutThunk,
...
describe('authSlice reducer', () => {
  ...
  it('handles setUser, clearUser, and clearError reducers', () => {
    const populated = reducer(initState(), setUser({ id: 7, role: 'customer' }));
  ...
  it('handles refresh token and initial hydration reducers', () => {
    ...
    const refreshRejected = reducer(seeded, { type: refreshTokenThunk.rejected.type });
    expect(refreshRejected.user).toBeNull();
    expect(refreshRejected.isAuthenticated).toBe(false);

    const hydrationPending = reducer(initState(), { type: fetchCurrentUserThunk.pending.type });
    ...
    const hydrationRejected = reducer(hydrationPending, { type: fetchCurrentUserThunk.rejected.type });
