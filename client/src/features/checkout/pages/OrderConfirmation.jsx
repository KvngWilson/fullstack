...
export default function OrderConfirmation() {
  ...
  const shouldVerify = useMemo(() => status === 'success' && Boolean(reference), [reference, status]);
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="mt-4 rounded-md border p-4">
        {isCancelled ? (
          <p className="text-sm text-destructive">{t('orderConfirmation.cancelled')}</p>
        ) : (
          <>
            ...
            {verificationStatus === 'loading' && (
              <p className="mt-2 text-sm text-muted-foreground">{t('orderConfirmation.verifying')}</p>
            )}
            ...
            {!shouldVerify && !isCancelled && (
              <p className="mt-2 text-sm text-muted-foreground">{t('orderConfirmation.successFallback')}</p>
            )}
          </>
        )}
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Link to="/account/orders" className="rounded-md border px-4 py-2 text-sm">
