		...
		<div className="mx-auto max-w-5xl px-4 py-8">
			...
			<div className="mt-8 grid gap-8 lg:grid-cols-[1fr_1.15fr]">
				<section className="rounded-lg border border-border bg-card p-6 shadow-sm">
					...
					{cards.length === 0 ? (
						<p className="mt-4 text-sm text-muted-foreground">{t('savedCards.empty')}</p>
					) : (
						<ul className="mt-4 space-y-3">
							{cards.map((card) => (
								<li key={card.id} className="rounded-md border border-border bg-background p-4">
				...
				<form onSubmit={handleAddCard} className="rounded-lg border border-border bg-card p-6 shadow-sm">
					<h2 className="text-xl font-semibold">{t('savedCards.addCard')}</h2>

					<div className="mt-5 grid gap-4">
						<label className="space-y-1">
							<span className="text-sm font-medium">{t('savedCards.fields.cardholder')}</span>
							<input
								type="text"
								name="holder"
								value={form.holder}
						...
						<label className="space-y-1">
							<span className="text-sm font-medium">{t('savedCards.fields.number')}</span>
							<input
								type="text"
								name="number"
								value={form.number}
						...
						<div className="grid gap-4 sm:grid-cols-2">
							<label className="space-y-1">
								<span className="text-sm font-medium">{t('savedCards.fields.expiryMonth')}</span>
								<input
									type="number"
									name="expiryMonth"
							...
							<label className="space-y-1">
								<span className="text-sm font-medium">{t('savedCards.fields.expiryYear')}</span>
								<input
									type="number"
									name="expiryYear"
						...
						<label className="space-y-1">
							<span className="text-sm font-medium">{t('savedCards.fields.cvc')}</span>
							<input
								type="password"
								name="cvc"
								value={form.cvc}
