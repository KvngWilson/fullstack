...
const normalizeGuestCartItem = (item = {}) => {
  const variantId = Number(
    item.productVariantId || item.variant_id || item.id || 0,
  );
...
const normalizeGuestCartResponse = (response = {}) => {
  const items = Array.isArray(response.items)
    ? response.items.map(normalizeGuestCartItem)
    : [];
...
const buildGuestAddPayload = (payload = {}) => {
  ...
  const price = Number(
    payload.price ?? payload.unit_price ?? payload.base_price ?? 0,
  );
  ...
  updateCartItem: async (payload) => {
    try {
      return await apiClient.patch(
        `/ordering/cart/items/${payload.cart_item_id}`,
        {
